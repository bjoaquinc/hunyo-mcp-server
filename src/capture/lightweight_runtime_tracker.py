...
class LightweightRuntimeTracker:
    ...
    def __init__(self, notebook_path: str = None, output_file: str = None):
        ...
        self.notebook_hash = get_notebook_file_hash(notebook_path) if notebook_path else "unknown"
        self.notebook_name = get_notebook_name(notebook_path) if notebook_path else "unknown"
        ...
        if output_file is None:
            if notebook_path:
                ...
                self.output_file = os.path.join(self.data_dir, "events", "runtime", "marimo_runtime.jsonl")
        else:
        ...
        print(f"🔍 Lightweight Runtime Tracker v2.0")
        print(f"📝 Notebook: {self.notebook_name} ({self.notebook_hash})")
        print(f"📁 Runtime logs: {self.output_file}")
    ...
    def end_cell_execution(self, execution_id: str, success: bool = True, error: Exception = None, result: Any = None) -> Dict:
        ...
        if self.process:
            try:
                ...
                memory_delta_mb = round(end_memory_mb - exec_ctx.get('start_memory_mb', 0), 2)
            except:
                pass
        ...
        if result is not None:
            try:
                ...
                result_info = {'capture_error': str(e), 'result_type': str(type(result).__name__)}
        ...
        try:
            ...
            if result is None:
                ...
                if length > 100:  # Large collection - describe only
                    element_types = list(set(type(x).__name__ for x in result[:10])) if result else []
                    result_info.update({
                        'length': length,
                    ...
                    result_info.update({
                        'length': length,
                        'preview': str_repr[:200] + "..." if len(str_repr) > 200 else str_repr,
                        'element_types': list(set(type(x).__name__ for x in result)) if result else []
                    })
                ...
                if 'matplotlib' in module_name and 'Figure' in type_name:
                    ...
                    try:
                        size_bytes = sys.getsizeof(result) if hasattr(sys, 'getsizeof') else 0
                        str_repr = str(result)
                        ...
                        if size_bytes > 10000 or len(str_repr) > 500:  # Large object
                            ...
                            result_info.update({
                                'string_repr': str_repr[:200] + "..." if len(str_repr) > 200 else str_repr,
                                'size_bytes': size_bytes
                            })
                    except:
                        result_info.update({
    ...
    def _emit_runtime_event(self, event: Dict):
        ...
        try:
            ...
            print(f"⚠️ Failed to emit runtime event: {e}")
    
    def get_session_summary(self) -> Dict:
        """Get summary of current session runtime data"""
        with self._lock:
            completed_executions = [
                exec_ctx for exec_ctx in self.cell_executions.values() 
                if 'end_time' in exec_ctx
            ]
            
            total_execution_time = sum(
                exec_ctx.get('duration_seconds', 0) 
                for exec_ctx in completed_executions
            )
            
            error_count = sum(
                1 for exec_ctx in completed_executions 
            ...
            return {
                ...
                'success_rate': round((len(completed_executions) - error_count) / max(len(completed_executions), 1), 2)
            }


# Global runtime tracker instance
_runtime_tracker: Optional[LightweightRuntimeTracker] = None
_tracker_lock = threading.Lock()


def enable_runtime_tracking(notebook_path: str = None, output_file: str = None) -> LightweightRuntimeTracker:
...
def get_runtime_tracker() -> Optional[LightweightRuntimeTracker]:
...
def disable_runtime_tracking():
    ...
    with _tracker_lock:
        if _runtime_tracker:
            print("🔍 Runtime tracking disabled")
...
def track_cell_execution(cell_source: str, cell_id: str = None):
    ...
    try:
        ...
        tracker.end_cell_execution(execution_id, success=False, error=e, result=exec_result.result)
        raise
    else:
        tracker.end_cell_execution(execution_id, success=True, result=exec_result.result)


if __name__ == "__main__":
    # Test the lightweight tracker
    print("🧪 Testing Lightweight Runtime Tracker")
    print("=" * 50)
    ...
    print("1. Testing simple execution with result capture...")
    ...
    print("2. Testing DataFrame operations with result capture...")
    with track_cell_execution("df = pd.DataFrame({'a': range(100), 'b': range(100)})") as ctx:
        import pandas as pd
    ...
    print("3. Testing complex result capture...")
    ...
    print(f"\n📋 Summary:")
    print(f"   Executions: {summary['total_cell_executions']}")
    print(f"   Success rate: {summary['success_rate']:.1%}")
    print(f"   Total time: {summary['total_execution_time_seconds']:.3f}s")
    
    print("\n✅ Lightweight tracker test completed!") 