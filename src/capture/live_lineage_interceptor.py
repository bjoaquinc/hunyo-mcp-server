...
# Import runtime tracker and utilities
try:
    from .lightweight_runtime_tracker import enable_runtime_tracking, track_cell_execution
    from . import get_notebook_file_hash, get_notebook_name, get_event_filenames, get_user_data_dir
    RUNTIME_TRACKING_AVAILABLE = True
    print("🔗 Runtime tracking integration enabled")
except ImportError:
    RUNTIME_TRACKING_AVAILABLE = False
    print("⚠️  Runtime tracking not available - running with basic lineage only")
...
class MarimoLiveInterceptor:
    ...
    def __init__(self, notebook_path: str = None, output_file: str = None, enable_runtime_debug: bool = True):
        # Determine notebook path and create unique file names
        if notebook_path is None:
            notebook_path = self._detect_notebook_path()
        
        self.notebook_path = notebook_path
        self.notebook_hash = get_notebook_file_hash(notebook_path) if RUNTIME_TRACKING_AVAILABLE and notebook_path else "unknown"
        self.notebook_name = get_notebook_name(notebook_path) if RUNTIME_TRACKING_AVAILABLE and notebook_path else "unknown"
        ...
        if RUNTIME_TRACKING_AVAILABLE:
            ...
            if output_file is None:
                if notebook_path:
                    _, self.output_file = get_event_filenames(notebook_path, self.data_dir)
                else:
                    self.output_file = str(Path(self.data_dir) / "events" / "lineage" / "marimo_live_lineage.jsonl")
            else:
        ...
        print(f"🎯 Marimo Live Interceptor v3.0")
        print(f"📝 Notebook: {self.notebook_name} ({self.notebook_hash})")
        print(f"📁 Lineage logs: {self.output_file.name}")
        print(f"🔧 Session: {self.session_id}")
        
        if self.enable_runtime_debug:
            self.runtime_tracker = enable_runtime_tracking(notebook_path=notebook_path)
            print(f"🔍 Runtime debugging enabled")
    ...
    def _detect_notebook_path(self) -> str:
        ...
        while frame:
            ...
            if filename.endswith('.py') and not filename.endswith('__init__.py'):
                ...
                if any(keyword in filename.lower() for keyword in ['notebook', 'marimo', 'analysis']):
                    return filename
            frame = frame.f_back
    ...
    def install(self):
        ...
        if self.interceptor_active:
            print("⚠️  Interceptor already active")
            return
        
        print("🔧 Installing live tracking hooks...")
    ...
    def _hook_builtin_exec(self):
        ...
        def monitored_exec(source, globals_dict=None, locals_dict=None):
            ...
            if isinstance(source, str) and globals_dict is not None and len(source.strip()) > 0:
                ...
                has_marimo_context = (
                    globals_dict and 
                    any(key.startswith('__') for key in globals_dict.keys()) and
                    len(globals_dict) > 10  # Typical marimo cell has many builtins
                )
                ...
                is_user_code = (
                    ...
                    has_reasonable_lines and
                    not_just_whitespace and
                    has_marimo_context
                )
            ...
            if isinstance(source, str) and globals_dict is not None and len(source.strip()) > 0:
                ...
                if is_user_code:
                    print(f"🔍 CAPTURING USER CODE: {source_preview}... (globals: {globals_count})")
                else:
                    # Show why we rejected it
                    rejection_reasons = []
                    if any(pattern in source for pattern in ['__dataclass_', 'def __create_fn__']):
                    ...
                    print(f"🔍 REJECTED: {source_preview}... ({', '.join(rejection_reasons)})")
            ...
            if is_user_code:
                ...
                _cell_execution_context.update({
                    ...
                    'globals_keys': list(globals_dict.keys()) if len(globals_dict) < 50 else f"<{len(globals_dict)} variables>"
                })
                ...
                try:
                    ...
                    if self.enable_runtime_debug and self.runtime_tracker:
                        with track_cell_execution(source, cell_id) as ctx:
                            ...
                            if globals_dict and '_' in globals_dict and globals_dict['_'] is not None:
    ...
    def _hook_builtin_compile(self):
        ...
        def monitored_compile(source, filename, mode, flags=0, dont_inherit=False, optimize=-1):
            # Debug compile calls
            if isinstance(source, str) and len(source.strip()) > 0:
                source_preview = source[:50].replace('\n', '\\n')
                print(f"🔍 COMPILE: {source_preview}... (filename: {filename}, mode: {mode})")
    ...
    def _hook_builtin_eval(self):
        ...
        def monitored_eval(expression, globals_dict=None, locals_dict=None):
            ...
            if isinstance(expression, str):
                ...
                globals_count = len(globals_dict) if globals_dict else 0
                print(f"🔍 EVAL: {expr_preview}... (globals: {globals_count})")
        ...
        try:
            ...
            for func_name in ['read_csv', 'read_json', 'read_excel', 'read_parquet']:
                if hasattr(pd, func_name):
                    ...
                    setattr(pd, func_name, self._create_read_wrapper(func_name, original_func))
            ...
            transformation_methods = [
                'merge', 'join', 'concat', 'groupby', 'drop', 'dropna', 
                'fillna', 'query', 'assign', 'pipe', 'transform', 'apply'
            ]
            ...
            for method_name in transformation_methods:
                if hasattr(pd.DataFrame, method_name):
                    ...
                    setattr(pd.DataFrame, method_name, 
            ...
            print("⚠️  pandas not available - DataFrame tracking disabled")
        except Exception as e:
            print(f"⚠️  pandas hooking failed: {e}")
    ...
    def _create_read_wrapper(self, func_name: str, original_func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                print(f"Error in {func_name} wrapper: {e}")
    ...
    def _create_method_wrapper(self, method_name: str, original_method):
        ...
        def wrapper(df_self, *args, **kwargs):
            try:
                ...
                print(f"Error in {method_name} wrapper: {e}")
    ...
    def _discover_new_dataframes(self, globals_dict: Dict[str, Any]):
        ...
        try:
            for var_name, obj in globals_dict.items():
                if (hasattr(obj, 'shape') and hasattr(obj, 'columns') and 
                    ...
                    if not already_tracked:
                        ...
                        self._track_dataframe_creation(obj, 'variable_assignment', name=var_name)
                    
        except Exception as e:
            print(f"Error discovering DataFrames: {e}")
    
    def _track_dataframe_creation(self, df, operation: str, args=None, kwargs=None, name=None):
        ...
        try:
            ...
            print(f"Error tracking DataFrame creation: {e}")
    
    def _track_dataframe_transformation(self, input_df, output_df, operation: str, args=None, kwargs=None):
        ...
        try:
            ...
            event = {
                ...
                'memory_delta_mb': round(
                    (output_df.memory_usage(deep=True).sum() - input_df.memory_usage(deep=True).sum()) / 1024 / 1024, 2
                ),
                'cell_context': dict(_cell_execution_context)
            }
            ...
            print(f"Error tracking DataFrame transformation: {e}")
    ...
    def uninstall(self):
        ...
        print("🔧 Uninstalling live tracking hooks...")
        ...
        try:
            ...
            if 'pandas_methods' in self.original_functions:
                ...
                for method_name in ['merge', 'join', 'concat', 'groupby', 'drop', 'dropna', 
                                  'fillna', 'query', 'assign', 'pipe', 'transform', 'apply']:
            ...
            self.interceptor_active = False
            print("✅ Live tracking uninstalled")
...
def enable_live_tracking(notebook_path: str = None, output_file: str = None, enable_runtime_debug: bool = True):
    ...
    if _global_interceptor is not None:
        print("⚠️  Live tracking already enabled")
        return _global_interceptor
    
    _global_interceptor = MarimoLiveInterceptor(notebook_path, output_file, enable_runtime_debug)
...
def disable_live_tracking():
    ...
    if _global_interceptor is not None:
        ...
        _global_interceptor = None
        print("🔇 Live tracking disabled")
    else:
        print("⚠️  Live tracking was not active")
...
if __name__ == "__main__":
    ...
    print("="*50)
    print(json.dumps(get_tracking_summary(), indent=2))
else:
