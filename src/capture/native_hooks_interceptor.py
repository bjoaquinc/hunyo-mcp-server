#!/usr/bin/env python3
"""
Marimo Native Hooks Interceptor - OpenLineage-compliant DataFrame lineage tracking
Captures pandas operations and generates proper OpenLineage events
"""
...
try:
    from marimo_lightweight_runtime_tracker import enable_runtime_tracking, track_cell_execution
...
class MarimoNativeHooksInterceptor:
    ...
    def __init__(self, lineage_file: str = "marimo_lineage_events.jsonl"):
        ...
        print(f"🎯 Marimo Native Hooks Interceptor v3.0 - OpenLineage")
        print(f"⚡ Runtime log: marimo_runtime.jsonl") 
        print(f"📊 Lineage log: {self.lineage_file.name}")
        print(f"🔧 Session: {self.session_id}")
        if self.runtime_tracker:
            print(f"🔍 Runtime tracking enabled")
    ...
    def install(self):
        ...
        if self.interceptor_active:
            print("⚠️  Native hooks already installed")
            return
        ...
        try:
            ...
            print("🔧 Installing marimo native hooks...")
            ...
            print("✅ Marimo native hooks installed!")
            print(f"   📁 Runtime log: marimo_runtime.jsonl")
            print(f"   📊 Lineage log: {self.lineage_file.name}")
            
        except ImportError as e:
            print(f"❌ Failed to import marimo hooks: {e}")
        except Exception as e:
            print(f"❌ Failed to install native hooks: {e}")
        ...
        try:
            ...
            for func_name in read_functions:
                if hasattr(pd, func_name):
                    ...
                    intercepted_func = self._create_pandas_reader_interceptor(func_name, original_func)
            ...
            intercepted_init = self._create_pandas_constructor_interceptor(original_init)
            ...
            methods_to_intercept = [
                ...
                # Data transformation  
                'apply', 'map', 'transform', 'aggregate', 'agg',
                # Data selection/filtering
                'query', 'filter', 'sample', 'head', 'tail',
                # Data reshaping
                'melt', 'stack', 'unstack', 'transpose',
                # Data combination
                'append', 'assign'
            ]
            ...
            for method_name in methods_to_intercept:
                if hasattr(pd.DataFrame, method_name):
                    ...
                    intercepted_method = self._create_pandas_interceptor(method_name, original_method, 'DataFrame')
            ...
            for func_name in module_functions:
                if hasattr(pd, func_name):
                    ...
                    intercepted_func = self._create_pandas_interceptor(func_name, original_func, 'pandas')
                    setattr(pd, func_name, intercepted_func)
            
            _pandas_intercepted = True
            print("🐼 Pandas interception installed for OpenLineage tracking")
            
        except ImportError:
            print("⚠️  Pandas not available - lineage tracking will be limited")
        except Exception as e:
            print(f"⚠️  Failed to install pandas interception: {e}")
    ...
    def _create_pandas_constructor_interceptor(self, original_init):
        ...
        def intercepted_init(df_self, *args, **kwargs):
            ...
            self._emit_openlineage_event('COMPLETE', run_id, job_name, inputs, outputs, current_execution_id)
    ...
    def _create_pandas_reader_interceptor(self, func_name: str, original_func):
        ...
        def intercepted_reader(*args, **kwargs):
            ...
            try:
                ...
                output_datasets = self._extract_output_datasets(result_df, source_path=source_path)
                
                self._emit_openlineage_event('COMPLETE', run_id, job_name, [], output_datasets, None, duration)
                
                return result_df

            except Exception as e:
                self._emit_openlineage_event('FAIL', run_id, job_name, [], [], None, error=str(e))
                raise
        
        return intercepted_reader
    
    def _create_pandas_interceptor(self, method_name: str, original_method, context: str):
        ...
        def intercepted_method(*args, **kwargs):
            ...
            self._emit_openlineage_event('START', run_id, job_name, input_datasets, [], current_execution_id)
            ...
            try:
                ...
                column_lineage = self._calculate_column_lineage(method_name, input_datasets, result, args, kwargs)

                # Extract output datasets
                output_datasets = self._extract_output_datasets(result, column_lineage=column_lineage)
                
                # Generate OpenLineage COMPLETE event
                self._emit_openlineage_event('COMPLETE', run_id, job_name, input_datasets, output_datasets, current_execution_id, duration)
                
                return result
                
            except Exception as e:
                # Generate OpenLineage FAIL event
                self._emit_openlineage_event('FAIL', run_id, job_name, input_datasets, [], current_execution_id, error=str(e))
                raise
    ...
    def _calculate_column_lineage(self, operation: str, inputs: List[Dict], output_df: Any, args, kwargs) -> Optional[Dict]:
        """
        ...
        output_fields = {}
        input_dfs = [df for df in args if hasattr(df, 'shape')] + [df for df in kwargs.values() if hasattr(df, 'shape')]
        ...
        try:
            if operation in ['merge', 'join']:
                ...
                for col in output_df.columns:
                    output_fields[str(col)] = {"inputFields": [{**input_id, "field": str(col)}]}
                ...
                for out_col in output_df.columns:
                    in_col = next((k for k, v in rename_mapping.items() if v == out_col), out_col)
                    output_fields[str(out_col)] = {"inputFields": [{**input_id, "field": str(in_col)}]}
                ...
                for col in output_df.columns:
                    if col in input_df.columns:
                        ...
                        all_input_fields = [{**input_id, "field": str(in_col)} for in_col in input_df.columns]
            ...
            print(f"⚠️  Could not calculate column lineage for '{operation}': {e}")
            return None
    
    def _extract_input_datasets(self, args: Tuple, kwargs: Dict, context: str) -> List[Dict]:
            ...
            if hasattr(value, 'shape') and hasattr(value, 'columns'):
                ...
                for j, item in enumerate(value):
                    if hasattr(item, 'shape') and hasattr(item, 'columns'):
                        dataset = self._dataframe_to_openlineage_dataset(item, f"input_{key}_{j}")
                        datasets.append(dataset)
        
        return datasets
    
    def _extract_output_datasets(self, result: Any, source_path: str = None, column_lineage: Optional[Dict] = None) -> List[Dict]:
        ...
        if hasattr(result, 'shape') and hasattr(result, 'columns'):
            ...
            dataset = self._dataframe_to_openlineage_dataset(result, "output", source_path, column_lineage)
            datasets.append(dataset)
        elif isinstance(result, (list, tuple)):
            # Multiple DataFrames
            for i, item in enumerate(result):
                if hasattr(item, 'shape') and hasattr(item, 'columns'):
                    dataset = self._dataframe_to_openlineage_dataset(item, f"output_{i}", source_path, column_lineage)
                    datasets.append(dataset)
        elif isinstance(result, dict):
            # Dictionary of DataFrames
            for key, value in result.items():
                if hasattr(value, 'shape') and hasattr(value, 'columns'):
                    dataset = self._dataframe_to_openlineage_dataset(value, f"output_{key}", source_path, column_lineage)
                    datasets.append(dataset)
        
        return datasets
    
    def _dataframe_to_openlineage_dataset(self, df, name_suffix: str = "", source_path: str = None, column_lineage: Optional[Dict] = None) -> Dict:
                ...
                "columnMetrics": {
                    ...
                    "columnMetrics": {
                        str(col): {
                            ...
                            "distinctCount": int(df[col].nunique()) if df[col].dtype != 'object' else None,
        ...
        if source_path:
            try:
                ...
                print(f"⚠️  Could not generate digest for {source_path}: {e}")
    ...
    def _emit_openlineage_event(self, event_type: str, run_id: str, job_name: str, 
                                inputs: List[Dict], outputs: List[Dict], 
                                execution_id: Optional[str] = None, duration: float = None, error: str = None):
        ...
        print(f"🔗 OpenLineage {event_type}: {job_name} (run: {run_id[:8]})")
    ...
    def _create_pre_execution_hook(self):
        ...
        def pre_execution_hook(cell, runner):
            ...
            try:
                ...
                cell_code = getattr(cell, 'code')
                
                # Start runtime tracking if available (non-blocking)
                execution_id = None
                if self.runtime_tracker:
                    try:
                        execution_id = self.runtime_tracker.start_cell_execution(cell_id, cell_code)
                    except Exception as rt_error:
                        print(f"⚠️  Runtime tracking error: {rt_error}")
                ...
                try:
                    print(f"⚠️  Error in pre-execution hook: {e}")
                except:
    ...
    def _create_post_execution_hook(self):
        ...
        def post_execution_hook(cell, runner, run_result):
            ...
            try:
                ...
                try:
                    success = not hasattr(run_result, 'exception') or run_result.exception is None
                ...
                if output is not None:
                    try:
                        result_info = self._capture_result_safely(output)
                    except Exception as cap_error:
                        result_info = {'capture_error': str(cap_error)}
                ...
                if self.runtime_tracker and execution_id:
                    try:
                        ...
                        print(f"⚠️  Runtime tracking end error: {rt_error}")
                ...
                try:
                    print(f"⚠️  Error in post-execution hook: {e}")
                except:
                    pass  # Even print might fail in some contexts
    ...
    def _create_finish_hook(self):
        ...
        def finish_hook(runner):
            ...
            try:
                ...
                print(f"⚠️  Error in finish hook: {e}")
    ...
    def _emit_lineage_event(self, event: Dict):
        ...
        try:
            ...
            print(f"⚠️  Failed to emit lineage event: {e}")
    ...
    def uninstall(self):
        ...
        try:
            ...
            print("🔧 Uninstalling marimo native hooks...")
            ...
            self.interceptor_active = False
            print("✅ Native hooks uninstalled")
            
        except Exception as e:
            print(f"⚠️  Error during hook uninstall: {e}")
    ...
    def _restore_pandas_methods(self):
        ...
        try:
            ...
            _pandas_intercepted = False
            print("🐼 Pandas methods restored")
            
        except Exception as e:
            print(f"⚠️  Error restoring pandas methods: {e}")
...
def enable_native_hook_tracking(lineage_file: str = "marimo_lineage_events.jsonl"):
    ...
    if _global_native_interceptor is not None:
        print(f"⚠️  Native hook tracking already enabled (session: {_global_native_interceptor.session_id})")
        return _global_native_interceptor
    
    print(f"🔧 Creating new interceptor instance...")
...
def disable_native_hook_tracking():
    ...
    if _global_native_interceptor is not None:
        ...
        _global_native_interceptor = None
        print("🔇 Native hook tracking disabled")
    else:
        print("⚠️  Native hook tracking was not active")


def is_native_tracking_active():
    """Check if native hook tracking is active"""
    return _global_native_interceptor is not None and _global_native_interceptor.interceptor_active
...
def get_native_tracking_summary():
    ...
    if _global_native_interceptor is not None:
        ...
        return {
            'status': 'inactive',
            'message': 'Native hook tracking is not enabled'
        }


# Auto-enable when imported (not in direct script execution)
if __name__ == "__main__":
    print("Manual test mode - run enable_native_hook_tracking() manually")
else:
    # Don't auto-enable to prevent startup issues - user must call enable_native_hook_tracking()
    print("🔧 Marimo hooks interceptor imported")
    print("🚨 IMPORTANT: Call enable_native_hook_tracking() to start tracking")
    print("   Example: import marimo_native_hooks_interceptor; marimo_native_hooks_interceptor.enable_native_hook_tracking()") 