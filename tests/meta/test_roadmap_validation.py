...
class TestServerImplementationPreparation:
    ...
    def test_project_structure_ready_for_server(self):
        ...
        # Updated for package separation - check packages directory
        mcp_server_dir = (
        ...
        assert (
            mcp_server_dir.exists()
        ), "hunyo_mcp_server package should exist in packages/ directory"
        assert (mcp_server_dir / "__init__.py").exists(), "Package should be importable"
        assert (mcp_server_dir / "config.py").exists(), "Config module should exist"

        # Subdirectories should exist for server integration
        assert (mcp_server_dir / "ingestion").exists(), "Ingestion package should exist"
        assert (mcp_server_dir / "tools").exists(), "Tools package should exist"
    ...
    def test_capture_layer_importable(self):
        ...
        # Add packages to path temporarily - updated for package separation
        capture_src_path = str(
            Path(__file__).parent.parent.parent / "packages" / "hunyo-capture" / "src"
        )
        if capture_src_path not in sys.path:
            sys.path.insert(0, capture_src_path)
        ...
        try:
            ...
            # Clean up path
            if capture_src_path in sys.path:
                sys.path.remove(capture_src_path)
    ...
    def test_config_system_ready(self):
        ...
        try:
            ...
            # Clean up path
            if mcp_src_path in sys.path:
                sys.path.remove(mcp_src_path)

    def test_pyproject_entry_point_configured(self):
        """Test that pyproject.toml has correct CLI entry point for server"""
        # Updated for package separation - check MCP server package config
        pyproject_path = (
            Path(__file__).parent.parent.parent
            / "packages"
            / "hunyo-mcp-server"
        ...
        assert pyproject_path.exists(), "hunyo-mcp-server pyproject.toml should exist"
    ...
    def test_dependencies_available_for_server(self):
        ...
        # Updated for package separation - check MCP server package dependencies
        pyproject_path = (
            Path(__file__).parent.parent.parent
            / "packages"
            / "hunyo-mcp-server"
        ...
        for dep in required_deps:
            assert (
                dep in content
            ), f"Required dependency {dep} should be in MCP server pyproject.toml"
