...
def test_capture_modules_importable():
    ...
    try:
        ...
        from capture import live_lineage_interceptor
        from capture import lightweight_runtime_tracker
        from capture import native_hooks_interceptor
        from capture import websocket_interceptor
        
        # Basic smoke test
        assert hasattr(logger, 'setup_logging')
        assert hasattr(live_lineage_interceptor, 'LiveLineageInterceptor')
