...
class TestHunyoMCPServerCLI:
    ...
    def temp_notebook_file(self, tmp_path):
        ...
        notebook = tmp_path / "test_notebook.py"
        notebook.write_text(
            """
import marimo as mo
import pandas as pd
...
mo.md(f"DataFrame shape: {df.shape}")
"""
        )
        return notebook
    ...
    def test_cli_requires_notebook_parameter(self, cli_runner):
        """Test that CLI requires --notebook parameter"""
        with patch.dict(
            "sys.modules",
            {
                "capture.logger": MagicMock(),
                "hunyo_mcp_server.orchestrator": MagicMock(),
                "hunyo_mcp_server.tools.lineage_tool": MagicMock(),
                "hunyo_mcp_server.tools.query_tool": MagicMock(),
                "hunyo_mcp_server.tools.schema_tool": MagicMock(),
            },
        ):
            # Import main after mocking problematic modules
    ...
    def test_cli_validates_notebook_exists(self, cli_runner, tmp_path):
        """Test that CLI validates notebook file exists"""
        with patch.dict(
            "sys.modules",
            {
                "capture.logger": MagicMock(),
    ...
    def test_mcp_server_configuration(self):
        ...
        # Since the server module has already been imported, we test the existing instance
        with patch.dict(
            "sys.modules",
            {
                "capture.logger": MagicMock(),
...
class TestServerIntegration:
    """Integration tests for server.py with partially mocked components"""
    ...
    def real_temp_notebook(self, tmp_path):
        """Create a realistic temporary notebook"""
...
data = pd.DataFrame({
    'user_id': range(100),
    'score': np.random.normal(85, 15, 100),
...
mo.md(f"Analysis complete. Categories: {data['category'].nunique()}")
"""
        )
        return notebook

    def test_error_propagation_chain(self, integration_runner, real_temp_notebook):
        ...
        with (
            ...
            # Simulate permission error in directory creation
            ...
            with pytest.raises(OSError, match="Permission denied"):
                integration_runner.invoke(
                    main,
                    ["--notebook", str(real_temp_notebook)],
